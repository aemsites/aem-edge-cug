# Akamai CUG Worker Setup

This setup enables Closed User Group (CUG) protection for AEM Edge Delivery sites served through Akamai. It deploys an EdgeWorker that sits between the browser and the AEM origin, authenticating users via Adobe IMS (OAuth 2.0 + PKCE) and enforcing per-path access control based on CUG headers published from AEM Author or Document Authoring (DA). Protected pages are never delivered to unauthenticated or unauthorized users — the EdgeWorker intercepts the origin response at the edge and gates access before any content reaches the browser.

## Demo Site

A working demo is available for each layer of the stack:

| Layer | URL |
|-------|-----|
| AEM Author | [author-p130360-e1560298.adobeaemcloud.com](https://author-p130360-e1560298.adobeaemcloud.com/ui#/aem/sites.html/content/aem-edge-cug-akamai) |
| Edge Delivery (origin) | [main--aem-edge-cug-akamai--aemsites.aem.live](https://main--aem-edge-cug-akamai--aemsites.aem.live/) |
| CDN (Akamai) | [aem-edge-cug-akamai.adobe.com](https://aem-edge-cug-akamai.adobe.com/) |

The origin serves pages without CUG enforcement. The Akamai CDN URL is where the EdgeWorker enforces authentication and group-based access control.
...
│          │     │              Akamai Edge                     │     │              │
│  Browser │────>│  ┌────────────────────────────────────────┐  │     │  Adobe IMS   │
│          │<────│  │  EdgeWorker (responseProvider)         │  │     │  (OAuth)     │
│          │     │  │  Auth + Proxy + CUG enforcement        │  │     │              │
│          │     │  │                                        │  │     └──────┬───────┘
│          │     │  │  Sessions: signed JWT in cookie        │  │            │
│          │     │  │  PKCE state: signed JWT in state param │  │            │
│          │     │  └───────────────┬────────────────────────┘  │            │
│          │     │                  │ httpRequest                │            │
│          │     │  ┌───────────────▼────────────────────────┐  │            │
│          │     │  │  Property Manager                      │  │            │
│          │     │  │  Routes sub-requests to origin or IMS  │──────────────┘
│          │     │  │  /ims/token/v3 → ims-na1.adobelogin   │  │
│          │     │  │  /* → AEM Edge Delivery origin         │──────────────┐
│          │     │  └────────────────────────────────────────┘  │            │
│          │     └──────────────────────────────────────────────┘            │
└──────────┘                                                     ┌──────────▼───────┐
                                                                 │  AEM Edge        │
...
Sub-requests from `httpRequest` go through Property Manager but do not trigger EdgeWorker events, so there is no recursion. The PM rule for `/ims/token/v3` forwards token exchange requests to `ims-na1.adobelogin.com` — this is required because EdgeWorker `httpRequest` can only reach hostnames served by the same Akamai property.

### Request Flow

1. Browser requests a page through Akamai Edge. The EdgeWorker intercepts the request.
2. `onClientRequest` blocks `/drafts/*` paths (404) and redirects non-standard ports.
3. `responseProvider` routes auth paths (`/auth/callback`, `/auth/login`, `/auth/logout`, `/auth/me`) to dedicated handlers.
4. RUM and media requests are proxied directly to origin with no auth check.
5. For all other requests, the EdgeWorker proxies to origin via `httpRequest` (routed through Property Manager to AEM Edge Delivery).
6. If the origin response includes `x-aem-cug-required: true` and no valid session cookie exists, the EdgeWorker redirects to Adobe IMS using OAuth Authorization Code + PKCE.
7. IMS authenticates the user and redirects back to `/auth/callback` with an authorization code.
8. The EdgeWorker exchanges the code for tokens via `httpRequest` to `/ims/token/v3`, which Property Manager forwards to IMS.
9. The EdgeWorker extracts user identity from the ID token, creates a signed JWT session cookie (`auth_token`), and redirects to the original page.
10. On subsequent requests, the EdgeWorker verifies the JWT cookie locally (HMAC-SHA256) and checks the user's email domain against `x-aem-cug-groups` from the origin response.

### Detailed Sequence

```
Browser                 EdgeWorker (responseProvider)     Property Manager          IMS                   Origin
  │                          │                               │                      │                      │
  │── GET /members/page ────>│                               │                      │                      │
  ...
  │                          │ encode verifier + URL in       │                      │                      │
  │                          │   signed JWT state param       │                      │                      │
  │<── 302 /authorize ───────│                               │                      │                      │
  │   ?code_challenge=...    │                               │                      │                      │
  │   &state=<signed JWT>    │                               │                      │                      │
  │                          │                               │                      │                      │
  ...
  │                          │                               │                      │                      │
  │── GET /auth/callback ───>│                               │                      │                      │
  │                          │ verify state JWT signature     │                      │                      │
  │                          │ extract PKCE verifier          │                      │                      │
  │                          │── httpRequest POST ───────────>│                      │                      │
  │                          │   /ims/token/v3               │── POST /ims/token/v3 ──>│                      │
  │                          │                               │   code=abc           │                      │
  │                          │                               │   code_verifier=...  │                      │
